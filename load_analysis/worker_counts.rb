...
if ['both','spot','demand'].include?(ARGV.first)
  WORKER_TYPES = ARGV.shift
else
  WORKER_TYPES = 'both'
end
...
SETS_TO_SHOW = [:launched_worker_output_capacity, :active_worker_output_capacity]
...
@workers = @cloud.workers.find(:all, :select => 'id,machine_type_id,state,created_at,alive_at,killed_at,updated_at,last_heartbeat_at,url,instance_id,spot', :conditions => ["(killed_at >= ? OR (killed_at is null AND updated_at >= ?)) AND created_at < ?", @start_time, @start_time, @end_time])
if ACCOUNT_ID != :all
...
@workers.each do |worker|
  ...
  next if WORKER_TYPES == 'spot' && !worker.spot?
  next if WORKER_TYPES == 'demand' && worker.spot?
